...
search_rf = RandomizedSearchCV(rf, space_rf, n_iter=216, scoring='accuracy', n_jobs=-1, cv=cv, random_state=1)
...
# define search space
space_gb = dict()
space_gb['learning_rate'] = [0.01, 0.1, 0.2]
space_gb['n_estimators'] = [100, 200]
space_gb['subsample'] = [0.8, 1.0, 1.2]
space_gb['min_samples_split'] = [2, 4, 6]
space_gb['min_samples_leaf'] = [1, 3, 5]
space_gb['max_depth'] = [3, 6, 9]
space_gb['max_features'] = [None, 'auto', 'sqrt']

# define Grid search
search_gb = GridSearchCV(gb, space_gb, scoring='accuracy', n_jobs=-1, cv=cv)

# define Random search 
search_gb = RandomizedSearchCV(gb, space_gb, n_iter=500, scoring='accuracy', n_jobs=-1, cv=cv, random_state=1)
# execute search
