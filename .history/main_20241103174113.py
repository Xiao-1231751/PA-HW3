...
class KNN:
    ...
    def prediction(self, X_test):
        predictions = []
        for x in X_test:
            distances = [self.calculate_distance(x, trainPoint) for trainPoint in self.x_train]  # Step 3
            k_indices = np.argsort(distances)[:self.k]  # Step 4
            k_nearest_labels = [self.y_train[i] for i in k_indices]  # Step 5
            most_common = Counter(k_nearest_labels).most_common(1)  # Step 6
            predictions.append(most_common[0][0]) 
        return predictions

knn_model = KNN(5)
knn_model.store(X_train, y_train)
...
print(f"The precision for KNN is {accuracy}")
